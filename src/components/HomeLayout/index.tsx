...
export const HomeLayout: React.FC = props => {
  ...
  return (
    <BaseLayout>
      <div className="grid grid-rows-[minmax(0,1fr),auto] grid-cols-1">
        <main className="w-full max-w-screen-lg px-4 mx-auto">
          <header className="my-16 flex flex-col justify-center">
            {heroImage &&
              (heroImage.includes('/') ? (
                <img
                  src={heroImage}
                  alt="hero"
                  className="max-w-full max-h-60 mx-auto mb-6"
                />
              ) : (
          ...
          {features && (
            <div className="grid grid-cols-[repeat(3,1fr)] gap-12 my-16 pt-12 border-t dark:border-dark-200 <sm:grid-cols-1">
